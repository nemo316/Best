...
-(void)setupRefreshView{
    ...
    self.subCategoryTableView.mj_footer = [WXHRefreshFooter footerWithRefreshingTarget:self refreshingAction:@selector(loadMoreDate)];
...
-(void)loadNewDate{
    ...
    params[@"a"] = @"list";
    params[@"c"] = @"subscribe";
    params[@"category_id"] = @(category.cate_id);
    params[@"page"] = @(1);
//    self.params = params;
    // 发送请求给服务器, 加载右侧的数据
    self.manager = [WXHHttpTool get:WXHCommonURL params:params success:^(id responseObj) {
